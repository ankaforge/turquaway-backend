...
class OpenAIJsonClient:
    ...
    def chat_json(
        ...
        timeout_seconds = float(
            timeout if timeout is not None else os.getenv("OPENAI_TIMEOUT_SECONDS", "45")
        )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    backoff_seconds *= 2
                    continue
                raise ValueError(f"OpenAI request timed out after {timeout_seconds}s: {exc}") from exc
